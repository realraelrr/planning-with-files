...
OPTIONAL_PARITY_SKILL_MD = [
    # clawhub-upload/ is generated for publication and intentionally ignored in
    # this repository. When present in a release workspace, it must match too.
...
class SkillMdVersionParityTests(unittest.TestCase):
    ...
    def test_all_parity_skill_md_share_canonical_version(self) -> None:
        ...
        optional_drift = []
        for rel in OPTIONAL_PARITY_SKILL_MD:
            path = REPO_ROOT / rel
            if not path.is_file():
                continue
            actual = read_skill_version(path)
            if actual != canonical:
                optional_drift.append((rel, actual))
        self.assertFalse(
            optional_drift,
            "Optional release artifact version drift detected. Run "
            f"`python scripts/bump-version.py {canonical}` to relock. "
            f"Out-of-sync: {optional_drift}",
        )
