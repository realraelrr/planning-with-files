...
Files touched (parity set, 18 tracked entries plus optional release artifact):
    ...
    CITATION.cff
    clawhub-upload/SKILL.md if that ignored release staging folder exists
...
OPTIONAL_PARITY_FILES = [
    # Generated/ignored release staging folder. Update it when present, but do
    # not fail normal source checkouts where it is intentionally absent.
...
def main(argv=None) -> int:
    ...
    for rel, kind in OPTIONAL_PARITY_FILES:
        path = REPO_ROOT / rel
        if not path.exists():
            print(f"  optional:  {rel}  (absent, skipped)")
            continue
        handler = HANDLERS[kind]
        old, err = handler(path, new, dry_run=args.dry_run)
        if err:
