...
function Resolve-PlanFile {
    ...
    $statePlan = Join-Path (Join-Path (Get-Location) ".state") "task_plan.md"
    if (Test-Path -LiteralPath $statePlan) {
        return (Resolve-Path -LiteralPath $statePlan).Path
    }
