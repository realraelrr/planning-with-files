...
#
# Usage:
#   ./init-session.sh                              # Codex default: .state/task_plan.md, findings.md, progress.md
#   ./init-session.sh [--template TYPE]            # Codex default with template choice
...
#
# Codex default mode (zero positional args, no --plan-dir) writes planning files
# under .state/ to keep operational agent state out of the project root. Slug
# mode addresses parallel multi-task isolation (issue #148) by writing each plan
# under .planning/<date>-<slug>/ and pinning .planning/.active_plan so
# resolve-plan-dir.sh can find it.
...
if [ "$SLUG_MODE" -eq 1 ]; then
    ...
    STATE_DIR="${PWD}/.state"
    mkdir -p "$STATE_DIR"
    create_files_in "$STATE_DIR"
    echo ""
    echo "Planning files initialized!"
    echo "Files: .state/task_plan.md, .state/findings.md, .state/progress.md"
fi
